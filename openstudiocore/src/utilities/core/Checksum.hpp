...
namespace openstudio {
  ...
  COREUTILITIES_API std::string checksum(const std::string& s);

  /// return 8 character hex checksum of istream
  COREUTILITIES_API std::string checksum(std::istream& is);

  /// return 8 character hex checksum of file contents
  COREUTILITIES_API std::string checksum(const path& p);
