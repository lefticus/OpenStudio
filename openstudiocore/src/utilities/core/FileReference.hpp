...
class COREUTILITIES_API FileReference {
 public:
...
namespace detail {
  ...
  COREUTILITIES_API QVariant toVariant(const FileReference& fileReference);

  /** Deserializes json variant to FileReference. */
  COREUTILITIES_API FileReference toFileReference(const QVariant& variant, const VersionString& version);
