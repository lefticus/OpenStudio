...
COREUTILITIES_API QVariant jsonMetadata();

/** Helper function to save top-level json files. */
COREUTILITIES_API bool saveJSON(const QVariant& json, openstudio::path p, bool overwrite);

/** Helper function to print top-level json files to string. */
COREUTILITIES_API std::string toJSON(const QVariant& json);

/** Helper function to load top-level json files. */
COREUTILITIES_API QVariant loadJSON(const openstudio::path& p);

/** Helper function to load top-level json data. Retrieved data is in .first,
 *  OpenStudio version of serialization is in .second. */
COREUTILITIES_API QVariant loadJSON(const std::string& json);

/** Returns the openstudio_version stored in the top-level JSON variant. */
COREUTILITIES_API VersionString extractOpenStudioVersion(const QVariant& variant);
