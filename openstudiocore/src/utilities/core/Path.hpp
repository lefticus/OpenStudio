...
namespace openstudio {
  ...
  COREUTILITIES_API path toPath(const wchar_t* s);

  /** UTF-16 encoded std::wstring to path*/
  COREUTILITIES_API path toPath(const std::wstring& s);

  // allow wpath to be written to cout on Windows
  COREUTILITIES_API std::ostream& operator<<(std::ostream& os, const path& p);
...
COREUTILITIES_API path tempDir();

/** path to std::string. */
COREUTILITIES_API std::string toString(const path& p);

/** path to QString. */
COREUTILITIES_API QString toQString(const path& p);

/** UTF-8 encoded char* to path*/
COREUTILITIES_API path toPath(const char* s);

/** UTF-8 encoded std::string to path*/
COREUTILITIES_API path toPath(const std::string& s);

/** QString to path*/
COREUTILITIES_API path toPath(const QString& q);
...
COREUTILITIES_API QString longPathName(const QString& path);
