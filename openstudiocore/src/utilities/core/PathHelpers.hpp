 ...
 *  subset thereof.) */
COREUTILITIES_API std::string modelFileExtension(); 
...
COREUTILITIES_API std::string componentFileExtension(); 

COREUTILITIES_API std::string tableFileExtension();

COREUTILITIES_API std::string documentFileExtension();

/** Single location for storing the default extension for rulesengine::Ruleset serialization files. */
COREUTILITIES_API std::string rulesetFileExtension();
...
COREUTILITIES_API path completePathToFile(const path& p,
                                      const path& base=path(),
                                      const std::string& ext=std::string(),
                                      bool warnOnMismatch=false);

/** Get p's extension as a string with the dot removed. */
COREUTILITIES_API std::string getFileExtension(const path& p);
...
COREUTILITIES_API path setFileExtension(const path& p,
...
COREUTILITIES_API bool makeParentFolder(const path& p,const path& base=path(),bool recursive=false);

/** Determines the relative path to p from base, if possible. Otherwise, returns empty path. */
COREUTILITIES_API path relativePath(const path& p,const path& base);

COREUTILITIES_API path completeAndNormalize(const path& p);

COREUTILITIES_API path relocatePath(const path& originalPath,
                                const path& originalBase,
                                const path& newBase);

/** Print information about path p available through boost::filesystem. */
COREUTILITIES_API std::ostream& printPathInformation(std::ostream& os,const path& p);

/** Recursively remove a directory, not subject to MAX_PATH */
COREUTILITIES_API bool removeDirectory(const path& dirName);

/** Recursively copy a directory, not subject to MAX_PATH */
COREUTILITIES_API bool copyDirectory(const path& source,
                                 const path& destination);

/** Determines if a directory is empty, returns false if directory does not exist, not subject to MAX_PATH */
COREUTILITIES_API bool isEmptyDirectory(const path& dirName);
