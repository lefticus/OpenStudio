...
namespace openstudio{
  ...
  COREUTILITIES_API int floor0( double value );
  ...
  class COREUTILITIES_API Time
  {
    public:
      friend COREUTILITIES_API std::ostream& operator<<(std::ostream& os, const Time& time);
  ...
  COREUTILITIES_API std::ostream& operator<<(std::ostream& os, const Time& time);

  /// TimeCompare compares two times, returns true if lhs is less than rhs
  class COREUTILITIES_API TimeCompare{
    public:
      /// operator()
