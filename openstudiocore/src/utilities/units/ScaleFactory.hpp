...
class COREUTILITIES_API ScaleFactorySingleton {
 ...
 public:
  ...
  friend COREUTILITIES_API std::ostream& operator<<(std::ostream& os,const ScaleFactorySingleton& factory);
...
COREUTILITIES_API std::ostream& operator<<(std::ostream& os,const ScaleFactorySingleton& factory);
...
COREUTILITIES_API std::string printScales();
...
COREUTILITIES_API std::pair<std::string,std::string> extractScaleAbbreviation(const std::string& str);
...
COREUTILITIES_API ScaleOpReturnType operator*(const Scale& firstScale,const Scale& secondScale);
 ...
 *  available, .second is amount final value should be multiplied by so result 
 *  will be correct when using .first. */
COREUTILITIES_API ScaleOpReturnType pow(const Scale& scale,int expNum,int expDenom=1);
...
COREUTILITIES_API ScaleOpReturnType scaleOpHelperFunction(int desiredExponent);
