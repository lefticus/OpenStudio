...
COREUTILITIES_API std::ostream& operator<<(std::ostream& os, const Unit& u);
 ...
 *  \sa UnitFactory */
COREUTILITIES_API Unit parseUnitString(const std::string& unitString);

/** Inequality for units. Does not compare scales or prettyStrings. */
COREUTILITIES_API bool operator!=(const Unit& lUnit,const Unit& rUnit);

/** Unit multiplication. Adds exponents on baseUnits and scales. Returned scale exponent
 *  may differ from expectation based on initialization of openstudio::ScaleFactory, see
 *  Scale operators declared in ScaleFactory.hpp. */
COREUTILITIES_API Unit operator*(const Unit& lUnit,const Unit& rUnit);
 ...
 *  based on initialization of openstudio::ScaleFactory, see Scale operators declared in
 *  ScaleFactory.hpp. */
COREUTILITIES_API Unit pow(const Unit& rUnit,int expNum,int expDenom=1);
